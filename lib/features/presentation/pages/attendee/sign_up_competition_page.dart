...
class SignUpCompetitionPage extends StatefulWidget {
  const SignUpCompetitionPage({super.key});

  @override
  _SignUpCompetitionPageState createState() => _SignUpCompetitionPageState();
}

class _SignUpCompetitionPageState extends State<SignUpCompetitionPage>{
  ...
  Widget build(BuildContext context) {
    ...
    if(isLoggedIn){
      ...
      create: (context) => sl()..add(LoadMainUserEvent(authState.uid)),
      child: BasicScaffold(
        ...
        return SizedBox(
          ...
          child: Column(
            children: [
              ...
              if (state.currentPage == 0) buildTeamBasicInfo(context, state),
              if (state.currentPage == 1) buildTeamMemberInfo(context, state),
              if (state.currentPage == 2) buildTeacherInfoWithFiles(context, state),
                  ...
                  if (state.currentPage == 2)
                    BasicWebButton(
                      onPressed: () => context
                          ...
                          .add(SubmitCompetitionFormEvent()),
                      title: '送出報名',
                      fontSize: 16,
                      width: 60,
                      height: 40,
